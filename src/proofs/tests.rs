...
struct TestValues {
    ...
    ciphertext: Vec<EncryptedAttribute>,
    blind_commitish: bls::G1Projective,
    blind: bls::Scalar,
...
impl TestValues {
    ...
    fn setup() -> Self {
        Self {
            ...
            commitish: bls::G1Projective::from(
                bls::G1Affine::from_compressed(&[
            ...
            ciphertext: vec![
                EncryptedAttribute {
                    value: (
                        bls::G1Projective::from(
                            bls::G1Affine::from_compressed(&[
                                0x96, 0xbb, 0x5f, 0xe1, 0x01, 0xc9, 0x69, 0x97, 0x3f, 0xb1, 0x74,
                                0x41, 0xae, 0x98, 0xcb, 0x99, 0x8e, 0x70, 0x50, 0x8d, 0x32, 0x29,
                                0x91, 0x5e, 0xaa, 0xb3, 0xba, 0xd6, 0x3a, 0xa4, 0x96, 0x49, 0xf4,
                                0x63, 0xaa, 0x0d, 0x20, 0x60, 0x38, 0x27, 0x76, 0x6d, 0x4b, 0x4a,
                                0xfc, 0xcd, 0x8a, 0x3c,
                            ])
                            .unwrap(),
                        ),
                        bls::G1Projective::from(
                            bls::G1Affine::from_compressed(&[
                                0xb9, 0xae, 0x56, 0x13, 0xd9, 0xbc, 0x2d, 0xb1, 0xeb, 0x82, 0x4d,
                                0x91, 0x99, 0xdc, 0x27, 0x33, 0xae, 0x68, 0x87, 0x2c, 0x8a, 0xfc,
                                0xed, 0xbe, 0x9c, 0x2f, 0x37, 0x19, 0x6a, 0xb3, 0xf9, 0x05, 0x8d,
                                0x57, 0xc9, 0x08, 0x79, 0x51, 0xf9, 0x5e, 0x2d, 0xae, 0x7d, 0x46,
                                0x90, 0xe6, 0xac, 0x42,
                            ])
                            .unwrap(),
                        ),
                ...
                EncryptedAttribute {
                    value: (
                        bls::G1Projective::from(
                            bls::G1Affine::from_compressed(&[
                                0x8d, 0x66, 0x78, 0xda, 0x17, 0xcc, 0x6f, 0xa1, 0x96, 0xa7, 0x7f,
                                0x6d, 0x0f, 0x29, 0x5f, 0x01, 0x34, 0x14, 0xec, 0x38, 0xf2, 0x81,
                                0xc6, 0xea, 0x0d, 0x83, 0xe6, 0xb8, 0x40, 0x63, 0x07, 0xf2, 0x9a,
                                0x6f, 0x24, 0x66, 0x5f, 0x22, 0x5f, 0xb4, 0xa3, 0x0c, 0xae, 0x09,
                                0x29, 0xa4, 0xdd, 0x45,
                            ])
                            .unwrap(),
                        ),
                        bls::G1Projective::from(
                            bls::G1Affine::from_compressed(&[
                                0x96, 0xfd, 0xfe, 0xf3, 0x56, 0x86, 0x6f, 0xa3, 0x82, 0x7a, 0x90,
                                0x9f, 0x76, 0xa8, 0xf2, 0x13, 0x27, 0xb7, 0x4a, 0xb3, 0xd7, 0x32,
                                0xc7, 0xb8, 0xbb, 0xf6, 0x7a, 0x29, 0xf6, 0x23, 0xc8, 0xfa, 0x3d,
                                0x93, 0x05, 0x05, 0xc8, 0x74, 0x7b, 0x75, 0x74, 0xa9, 0x4e, 0x7b,
                                0x03, 0x49, 0x5c, 0xbc,
                            ])
                            .unwrap(),
                        ),
            ...
            blind_commitish: bls::G1Projective::from(
                bls::G1Affine::from_compressed(&[
...
fn test_signature_request_proof() {
    ...
    let values = TestValues::setup();

    //
    // Signing steps
    //
    // random k
    let witness_blind = Rc::new(Witness::new(&params, values.blinding_factor.clone()));
    let witness_attributes: Vec<_> = chain(&values.private_attributes, &values.public_attributes)
        .map(|attribute| Rc::new(Witness::new(&params, attribute.clone())))
        .collect();
    let witness_keys: Vec<_> = values
        .attribute_keys
        .iter()
        .map(|attribute| Rc::new(Witness::new(&params, attribute.clone())))
        .collect();
    let attribute_indexes: Vec<_> = chain(&values.private_attributes, &values.public_attributes)
        .enumerate()
    ...
    // R = k G
    let commitments =
        proof_builder.commitments(&values.gamma, &values.commitish, &values.attribute_commit);
    ...
    let proof = signature_proof::Proof {
        response_blind,
        response_attributes,
        response_keys,
    };
...
fn test_credential_proof() {
    ...
    //
    // random k
    let witness_attributes: Vec<_> = values
        .private_attributes
        .iter()
        .map(|attribute| Rc::new(Witness::new(&params, attribute.clone())))
        .collect();
    let witness_blind = Rc::new(Witness::new(&params, values.blind.clone()));
    let attribute_indexes: Vec<_> = values
        .private_attributes
        .iter()
        .enumerate()
        .map(|(index, _)| index as u64)
    ...
    let proof_builder = credential_proof::Builder::new(
        &params,
        witness_attributes.clone(),
        witness_blind.clone(),
        attribute_indexes.clone(),
    ...
    // R = k G
    let commitments = proof_builder.commitments(&values.verify_key, &values.blind_commitish);
    ...
    let proof = credential_proof::Proof {
        response_attributes,
        response_blind,
    };
